<template>
  <div>
    <h1>Custom reactive template</h1>
    <p>Below you can see a custom reactive template in action.</p>
    <vue-dropzone ref="myVueDropzone" id="dropzone"
      ...
      :duplicateCheck="true">
      <template v-slot:template="{ file, index }">
        <div style="border:1px solid #ddd;width:25%;display:inline-block;">
        <img :src="file.dataUrl" />
        {{ file.upload.filename }}
        <progress :value="file.upload.progress / 100"></progress>
        <code>{{ file.upload.progress.toFixed() }}</code>
